...
//===================================================================================================================
    ...
    void initMPU9250()
    {
        ...
        // Configure Gyro and Accelerometer for low pass filter .
        ...
        c = c & ~0x03; // Clear Fchoice bits [1:0] //  0x03 <-- 0x2 low pass filter selection .
        c = c & ~0x18; // Clear AFS bits [4:3]
        c = c | Gscale << 3; // Set full scale range for the gyro : 250DPS
        ...
        // c = c & ~0xE0; // Clear self-test bits [7:5]
        c = c & ~0x18;  // Clear AFS bits [4:3]
        c = c | Ascale << 3; // Set full scale range for the accelerometer : 2G
        ...
        writeByte(MPU9250_ADDRESS, INT_PIN_CFG, 0x10); //  Interrupt status is cleared if any read operation is performed.
    ...
    void calibrateMPU9250(float *dest1, float *dest2)
    {
        ...
        // set enable to accelerometer , gyrometer .
        ...
        writeByte(MPU9250_ADDRESS, CONFIG, 0x01);      // Set low-pass filter to 184 Hz ( bandwidth )
        writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x00);  // Set sample rate to 1 kHz
        writeByte(MPU9250_ADDRESS, GYRO_CONFIG, 0x00);  // FS_SEL=0, Set gyro full-scale to 250 degrees per second, maximum sensitivity
        writeByte(MPU9250_ADDRESS, ACCEL_CONFIG, 0x00); // AFS_SEL=0, Set accelerometer full-scale to 2 g, maximum sensitivity

        uint16_t gyrosensitivity = 131;   // = 131 LSB/degrees/sec for FS_SEL=0
        uint16_t accelsensitivity = 16384;  // = 16384 LSB/g  for AFS_SEL=0
        ...
        packet_count = fifo_count / 12;// How many sets of full gyro and accelerometer data for averaging
                                        // mag 에 대한 자료 설정이 없으므로 gyro와 accelerometer만 고려한다.
        ...
        data[0] = (-gyro_bias[0] / 4 >> 8) & 0xFF; // Divide by 4 to get 32.75 LSB per deg/s to conform to expected bias input format
        ...
        */
        ...
        // device 가 startup되면, write된 내용이 사라지므로 write하지 않는다.
        // 대신 아래 처럼, 함수의 인자로 전달한다.
...
// Configure the accelerometer for self-test
        ...
        {
            destination[i] =
                    100.0 * ((float) (aSTAvg[i] - aAvg[i])) / factoryTrim[i] - 100.; // Report percent differences
            destination[i + 3] =
...
// but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
    ...
    // measured ones.
    void
    MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
    {
        float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
        float norm;
        float hx, hy, bx, bz;
